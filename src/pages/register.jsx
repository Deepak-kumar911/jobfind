...
export const Register = () => {
  const navigate = useNavigate()
    ...
    const handleSubmit =async (e) =>{
        ...
        if(data.role==="employee"){
          const res = await authUser(data,"employee","register");
          if(res){
            navigate("/")
          }
      }else if(data.role==="employer"){
        const res = await authUser(data,"employer","register");
        if(res){
          navigate("/")
        }
      }else{
  ...
  return (
    ...
    <div className='flex flex-col bg-white border-blue-600 w-[90%] md:w-[30%] p-4 rounded-lg'>
        <h4 className='text-center text-indigo-800 mb-4'>Register for FindJob</h4>
        ...
        <input type="text" name="email" onChange={handleOnchange} value={data.email} className='border-b-2 rounded-md border-indigo-600 w-[100%] px-2 py-1 outline-none' placeholder='Email ID' id="email" />
            ...
            <select name="role" id="role" onChange={handleOnchange} value={data.role} className='border-b-2 rounded-md border-indigo-600 w-[100%] px-2 py-1 outline-none'>
             ...
             <button type="submit" className='w-[100%]  py-2 bg-violet-600 rounded-md text-white text-lg hover:bg-orange-400 mt-4'>Register</button>
        </form>
        <div className='mt-3 text-center'>
            <h6 className='text-indigo-500'>Already have a Account ?</h6>
            <NavLink to="/login" className="flex no-underline"><div className='w-[100%]  py-2 bg-green-400 rounded-md text-white text-lg hover:bg-red-500'>Login</div></NavLink>
            </div>
    </div>
</div>
  )
